...
/**
 デリゲートへ位置情報の推定結果を通知します。
 ポジションログの生成を行っている時のみ発生します。
 @param result 結果が保存された辞書配列
            result[@"cluster"] 現在位置しているクラスター(BCLCluster型)
            result[@"nearest"] 最寄りのビーコン情報(BCLBeacon型)
            result[@"x"] 推定したクラスタ内のX座標(NSNumber double型)
            result[@"y"] 推定したクラスタ内のY座標(NSNumber double型)
            result[@"h"] 推定した高さ(NSNumber double型)
            result[@"r"] 誤差半径(NSNumber double型)
            result[@"numberOfBeacons"] 位置推定に使用したビーコンの数
            result[@"timestamp"] 位置推定を行った時刻(UNIX時間)
 */
- (void)didEstimatePosition:(NSDictionary *)result;

/**
 デリゲートへ現在位置しているクラスターに変化があった事を通知します。
 ポジションログの生成を行っている時のみ発生します。
 @param cluster 現在位置しているクラスター。クラスタが不明な場合nilを返します。
 */
- (void)didChangeLocatedCluster:(BCLCluster *)cluster;
...
/**
 ポジションログを送信する間隔を取得します。
 @return ポジションログ送信間隔（秒）
 */
- (NSInteger)getPositionLogInterval;

/**
 ポジションログを送信する間隔を設定します。
 @param interval ポジションログ送信間隔（設定できる範囲は1〜3600秒の間となります）
 */
- (void)setPositionLogInterval:(NSInteger)interval;
